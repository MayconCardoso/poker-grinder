...
internal class ChartMoneyVariationController {
   ...
   */
  private val circleMarkerRadius by lazy { 3.dp().toFloat() }
  ...
  internal fun setData(data: List<MoneyVariationEntry>) {
    ...
    lowestDataValue = data.minBy { it.amount }.amount
    highestDataValue = data.maxBy { it.amount }.amount
  }
  ...
  private fun circleSizeOffset() = circleMarkerRadius.pixel() * 2
